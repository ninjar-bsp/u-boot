...
struct ssd1327_operations {
    ...
    int (*clear)(struct ssd1327_par *par, int clear);
    ...
    int (*set_cursor)(struct ssd1327_par *par, int x, int y);
};
...
static int ssd1327_spi_write(struct ssd1327_par *par, void *buf, size_t len)
{
    return dm_spi_xfer(par->dev, (len * BITS_PER_BYTE),
                       buf, NULL, SPI_XFER_ONCE);
}

static int write_buf_dc(struct ssd1327_par *par, void *buf, size_t len, int dc)
{
    int ret;
    ...
    ret = ssd1327_spi_write(par, buf, len);
    if (ret)
        dev_err(par->dev, "write() failed and returned %d\n", ret);
    return ret;
}

static int request_one_gpio(struct ssd1327_par *par,
...
static int request_gpios(struct ssd1327_par *par)
{
    int ret;
    printk("%s, ============\n", __func__);
...
static inline int ssd1327_write_byte(struct ssd1327_par *par, u8 byte, int dc)
{
...
static int ssd1327_clear(struct ssd1327_par *par, int clear)
{
    int i;
    ...
    dm_gpio_set_value(par->gpio.dc, 1);
    for (i = 0; i < width * height * 16 / 8; i++)
        ssd1327_spi_write(par, &clear, 1);
    return 0;
}
...
static const struct ssd1327_display default_ssd1327_display = {
    .xres = 128,
    .yres = 128,
    .bpp = 4,
    .fps = 60,
